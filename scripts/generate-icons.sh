...
if ! command -v sips &> /dev/null; then
    echo "❌ Error: sips not found (macOS required for icon generation)"
    exit 1
fi
...
if command -v rsvg-convert &> /dev/null; then
    ...
    echo "⚠️  No SVG converter found. Install librsvg: brew install librsvg"
...
if [ ! -f "$BUILD_DIR/icon.png" ]; then
    echo "❌ Could not create PNG from SVG"
    echo "   Please install librsvg: brew install librsvg"
    exit 1
fi

echo "✅ Created master PNG (1024x1024)"
